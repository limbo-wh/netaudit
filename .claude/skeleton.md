...
└── NetAudit/
    ...
    │   │   ├── RamInfo.cs                паспорт памяти: модули, слоты, каналы, состояние
    │   │   ├── RamInfoProbe.cs           разбор сырых таблиц SMBIOS плюс счётчики
    │   │   │                             занятости, подкачки и сжатой памяти
    ...
    │   │   ├── RamDiagnosticTest.cs      сводная диагностика ОЗУ: паспорт, настройка,
    │   │   │                             состояние, журналы сбоев, замеры и выводы
    │   │   ├── RamBenchmark.cs           скорость чтения/записи/копирования, задержка
    │   │   │                             и лесенка кэшей процессора
    │   │   ├── RamIntegrityTest.cs       проверка ОЗУ шаблонами (MemTest из-под Windows)
...
- **Оперативная память (2026-09-14):** полная диагностика ОЗУ — паспорт планок
  из сырых таблиц SMBIOS (модель, ранги, напряжение, каналы), определение
  невключённого профиля XMP, разбор текущей занятости со сжатой памятью и
  подкачкой, следы сбоев по памяти в журналах Windows, замеры скорости и
  задержки, проверка шаблонами.
