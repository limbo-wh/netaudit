...
скорее всего, дело в портативной сборке: проводник Windows помечает каждый
файл из распакованного zip как «из интернета», и именно эта метка, а не сама
программа, включает жёсткую блокировку. Правой кнопкой по `NetAudit.App.exe`
→ Свойства → галочка «Разблокировать» внизу окна → OK, и запустится. Установщик
обычно этой проблемы не создаёт вовсе.
