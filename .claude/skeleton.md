...
└── NetAudit/
    ...
    │   │   ├── CpuInfo.cs                паспорт процессора: ядра, кэши, наборы инструкций
    │   │   ├── CpuInfoProbe.cs           сбор паспорта: реестр, раскладка ядер, кэши,
    │   │   │                             гипервизор, схема питания
    │   │   ├── CpuAffinity.cs            привязка потока к ядру и замер настоящей частоты
    ...
    │   │   ├── CpuDiagnosticTest.cs      сводная диагностика ЦП: паспорт, состояние,
    │   │   │                             журналы сбоев, замеры и выводы
    │   │   ├── CpuBenchmark.cs           целые, плавающая точка, шифрование, прирост
    │   │   │                             по потокам, частота в бусте, троттлинг
    │   │   ├── CoreLatencyTest.cs        задержки обмена между ядрами, матрица пар
...
- **Процессор (2026-09-14):** полная диагностика ЦП — паспорт (поколение по
  семейству и модели, кэши по уровням с разбиением на кластеры, наборы инструкций,
  микрокод), состояние (частота, схема питания, гипервизор, защита на
  виртуализации), следы сбоев по процессору в журналах, замеры (целые, FMA, AES,
  прирост по потокам, частота в бусте своим замером тактов, троттлинг) и задержки
  обмена между ядрами матрицей.
